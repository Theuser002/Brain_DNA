...
if __name__ == "__main__":
    ...
    eval_file = open(clf_cfg['RF_EVALUATION_RESULTS'], 'w')
    ...
    for group in tqdm(groups, desc = 'Groups: ', position = 0):
        for fold in tqdm(trained_folds, desc = 'Folds: ', position = 1):
            ...
            clf = RandomForestClassifier(max_depth=None, criterion='log_loss', oob_score=True, random_state=42, verbose=0)
            clf.fit(train_features, train_labels_int)
            val_acc = clf.score(val_features, val_labels_int)
            
            eval_str = f'\n>>>>>>{group.upper()}] - {fold} - val acc: {val_acc}\n'
            tqdm.write(eval_str)
            eval_file = open(clf_cfg['RF_EVALUATION_RESULTS'], 'a+')
            ...
            if save == 'save':
                ...
                MODEL_PATH = os.path.join(clf_cfg['RF_MODELS_DIR'], group, f'{fold}.joblib')
